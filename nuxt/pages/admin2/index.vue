<template>
    ...
    <div v-else class="m-4 flex flex-row space-x-8">
      <div v-if="users.length === 0" class="w-full">
        <p class="text-center text-xl">
          No hi ha cap usuari registrat a l'aplicació.
        </p>
      </div>
      <div v-else>
        <div class="width flex flex-col justify-center ml-auto mr-auto gap-3">
          <div class="relative text-center w-full flex flex-row">
            ...
            <Transition name="delete-fade">
              ...
              >
                <span
                  class="material-symbols-rounded p-1 hover:bg-gray-400/[.25] rounded-full"
                >
                  Close
                </span>
              </button>
            ...
            <button
              ...
              id="buttonFilters"
              @click="isFiltersSlideOpen = !isFiltersSlideOpen"
            >
              <span class="material-symbols-outlined text-white"> tune </span>
            </button>
          </div>
          <UButton
            :color="isSelected(user) ? 'black' : 'sky'"
            :variant="isSelected(user) ? 'solid' : 'soft'"
          ...
          >
            ...
            <UIcon
              v-if="user.vote_banned_until || user.propose_banned_until"
  ...
  <USlideover v-model="isFiltersSlideOpen" class="z-[9999]">
    <UCard
      ...
      :ui="{
        body: { base: 'flex-1', background: 'bg-stone-800' },
        ring: '',
    ...
    >
      ...
      <div class="bg-stone-800 p-4 rounded-lg">
        ...
        >
          <h3 class="text-2xl font-semibold text-white">FILTRES</h3>
          <button
            @click="isFiltersSlideOpen = false"
            class="h-8 w-8 flex items-center justify-center hover:bg-stone-600 rounded-full p-2"
          >
            <span
              class="i-heroicons-x-mark-20-solid h-5 w-5"
              aria-hidden="true"
            ></span>
          </button>
        </div>
        <div class="pt-4">
          <div class="flex items-center justify-between pb-2">
            <h1 class="text-xl text-white">Ordre</h1>
            <div class="flex space-x-4">
              <button
                @click="orderBy = 'name'"
                :class="{
                  'font-semibold text-sky-500 bg-sky-600': orderBy === 'name',
                }"
              ...
              <!-- <button
                @click="orderBy = 'group'"
                :class="{
                  'font-semibold text-sky-500 bg-sky-600': orderBy === 'group',
                }"
                class="py-2 px-4 rounded-md bg-gray-700 hover:bg-sky-600 text-white transition duration-300 ease-in-out"
              >
                Grup
              ...
              <button
                @click="orderBy = 'block'"
                :class="{
                  'font-semibold text-sky-500 bg-sky-600': orderBy === 'block',
                }"
                class="py-2 px-4 rounded-md bg-gray-700 hover:bg-sky-600 text-white transition duration-300 ease-in-out"
              >
                Sancionats
        ...
        <div v-if="classGroups.length > 0" class="pt-4">
          <div class="flex items-center justify-between pb-2">
            <h1 class="text-xl text-white">Grups</h1>
            <button
              @click="selectGroup(null)"
              class="py-2 px-4 rounded-md bg-gray-700 hover:bg-sky-600 text-white transition duration-300 ease-in-out"
              :class="{
                'font-semibold text-sky-500 bg-sky-600': filterGroup == null,
              }"
            >
              Tots
            </button>
          ...
          <div class="flex flex-wrap -mx-2">
            <div
              ...
              :key="index"
              class="w-32 px-2 mb-2"
            >
              <button
                @click="selectGroup(group.id)"
                :class="
                  filterGroup === group.id
                    ? 'border-sky-600 text-sky-500 bg-sky-600'
                    : ''
                "
      ...
      if (this.orderBy) {
        filteredUsers.sort((a, b) => {
          if (this.orderBy === "name") {
            ...
            const groupA = a.groups.length > 0 ? a.groups[0].name : "";
            const groupB = b.groups.length > 0 ? b.groups[0].name : "";

            return groupA.localeCompare(groupB);
          } else if (this.orderBy === "block") {
            const aBanned =
              a.vote_banned_until || a.propose_banned_until ? 1 : 0;
            const bBanned =
              b.vote_banned_until || b.propose_banned_until ? 1 : 0;

            return bBanned - aBanned; // Los baneados van primero
          }
        });
      }

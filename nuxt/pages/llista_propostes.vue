<template>
    ...
    <Vinyl class="fixed top-0 right-0 h-screen flex justify-center items-center overflow-hidden translate-x-[40%]" v-if="mobileDetector === 0" />
    ...
    <div v-if="checkVotingState === 'vote'" class="mb-4">
        ...
        <div class="w-full flex items-center gap-2" :class="{ 'w-[57%] ml-12': !$device.isMobile, 'px-3': $device.isMobile }">
        ...
        <h2 class="my-4 text-3xl font-bold" :class="{'m-2 ml-12 mt-4': !$device.isMobile, 'text-center': $device.isMobile}">Cançons proposades</h2>
        <!-- <TransitionGroup tag="div" class="mb-20" name="song-slide" mode="out-in"> -->
        <div v-if="songs.length === 0" class="mt-4 w-full">
            <p class="text-xl" :class="{'ml-12': !$device.isMobile, 'text-center': $device.isMobile}">Encara no s'ha proposat cap cançó.</p>
            <p class="mx-4" :class="{'ml-12': !$device.isMobile, 'text-center': $device.isMobile}">Anima't a compartir la teva proposta fent <br v-if="$device.isMobile">
                cercant a
                la barra de búsqueda.</p>
        </div>

        <div v-if="songs.length != 0 && filteredSongs.length === 0" class="mt-4 w-full">
            <p class="text-xl"  :class="{'ml-12': !$device.isMobile, 'text-center': $device.isMobile}">No hi ha cap cançó proposada amb aquesta cerca.</p>
            <p class="mx-4"  :class="{'ml-12': !$device.isMobile, 'text-center': $device.isMobile}">Comparteix la teva proposta buscant ara mateix!</p>
        </div>
        ...
        <Transition name="song-slide">
            <h2 v-if="spotifySongs.length > 0" class="text-3xl font-bold mt-10" :class="{'ml-12': !$device.isMobile, 'text-center': $device.isMobile}">Resultats de la cerca</h2>
        </Transition>

<template>
    ...
    <div v-if="checkVotingState === 'vote'">
        <div v-if="settings.theme" class="flex justify-center content-center my-8">
            <h1 class="text-5xl smallCaps w-fit text-center">
        ...
        <div class="w-[57%] ml-12 flex items-center gap-2" :class="{ 'w-[90%]': $device.isMobile }">
        ...
        <div v-if="songs.length === 0" class="mt-4 w-full">
            <p class="ml-12 text-xl">Encara no s'ha proposat cap cançó.</p>
            <p class="ml-12 mx-4">Anima't a compartir la teva proposta fent <br v-if="$device.isMobile">
                cercant a
                la barra de búsqueda.</p>
        </div>

        <div v-if="songs.length != 0 && filteredSongs.length === 0" class="mt-4 w-full">
            <p class="ml-12 text-xl">No hi ha cap cançó proposada amb aquesta cerca.</p>
            <p class="ml-12 mx-4">Comparteix la teva proposta buscant ara mateix!</p>
        </div>

        <!-- CANCIONES PROPUESTAS -->
        <TransitionGroup name="song-slide" mode="out-in">
            <div class="w-[60%] mb-4" v-for="track in filteredSongs" :key="track.id">
        ...
        <Transition name="song-slide">
            <h2 v-if="spotifySongs.length > 0" class="ml-12 text-3xl font-bold mt-10">Resultats de la cerca</h2>
        </Transition>
        <TransitionGroup tag="div" class="w-[60%]" mode="out-in" name="song-slide">
...
export default {
    data() {
        return {
            ...
            activePlayer: {
                0: resolveComponent('ModularPlayer2'),
